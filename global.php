<?php
$BASE = 'base'; 		                    // (Optional) The parent directory you want to inherit all properties except what you changed
// $ASEQ = '[the-carrent-directory-name]'; 	// (Optional) The current subdomain sequence, or leave null if this file is in the root directory
// $SEQUENCES_PATCH = []; 		            // (Optional) An array to apply your custom changes in \_::$Sequence
                                            // newdirectory, newaseq; // Add new directory to the \_::$Sequence
                                            // directory, newaseq; // Update directory in the \_::$Sequence
                                            // directory, null; // Remove thw directory from the \_::$Sequence
?>